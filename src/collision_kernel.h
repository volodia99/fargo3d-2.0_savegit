  ...
  for (p=0; p<NFLUIDS; p++) {
    ...
    if (p != o) {
	 ...
	 have the possibility of disabling feedback if necessary.*/
      ...
      #ifdef Z    // 3D
      #ifdef ISOTHERMAL
      cs3d  = 0.5*(cs[l] + cs[idm]);
      #else     // ADIABATIC
      cs3d  = 0.5*(sqrt(GAMMA*(GAMMA-1.0)*cs[l]/rho[0][l]) + sqrt(GAMMA*(GAMMA-1.0)*cs[idm]/rho[0][idm]));
      #endif
      stokestosize = rho_gas*cs3d/omega;
      #else       // 2D
      stokestosize = rho_gas;
      #endif
      if ( p > o )  m[p+o*NFLUIDS] = -dt*omega*stokestosize*alpha[p+o*NFLUIDS]*rho_p/rho_o;
      else          m[p+o*NFLUIDS] = -dt*omega*stokestosize*alpha[p+o*NFLUIDS];
#endif
    ...
    stokestosize = rho_gas;
    #endif
    if( q > p ) sum += stokestosize*alpha[q+p*NFLUIDS]*rho_q/rho_p;
    else        sum += stokestosize*alpha[q+p*NFLUIDS];
#endif
